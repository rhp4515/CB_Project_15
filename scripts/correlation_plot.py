...
def errorFracPlots(D, measure, methods, simIDs, outdir, plotname):
    sns.set_style('white')
    sns.set_context('poster')
    plt.clf()
    plt.cla()
...
def makeTable(methodDict, outpath, outfile, measure, annotPath, isFlux=True):
    import pandas as pd
    import seaborn as sns
    import ParsingUtils
    import AnalysisUtils
    ...
    for k, v in methodDict.items():
        if k.upper().startswith('SALMON'):
            ...
            print ("reading sailfish quasi")
            d = ParsingUtils.readSalmon(v, '_{}'.format(k))
        elif k.upper() == 'TIGAR':
            print ("reading tigar")
            d = ParsingUtils.readTigar(v, '_{}'.format(k))
        elif k.upper() == 'RSEM (QUANT)':
            import os
            print ('reading rsem data from {}'.format(os.path.abspath(v)))
            d = ParsingUtils.readRSEMTruth(v, '_{}'.format(k))
            ...
            if isFlux:
                ...
                # Flux sim thinks paired-end = 2 reads . . .
                d["NumReads{}".format(suffix)] = d["SeqNum{}".format(suffix)] * 0.5
            else:
                suffix = '_{}'.format(k)
                d = ParsingUtils.readRSEMTruth(v, suffix)
    ...
    for mn in set(methodDict.keys()) - set(["Truth", "eXpress"]):
        ...
        minVal = min(minVal, newMin)
    print("Not filtering eXpress results < {} {}".format(minVal, measure))
    ...
    wmrdName = 'weighted abs. rel. diff'
    corrName = 'Spearman corr.'
    propName = 'Proportionality corr.'
    tpefName = 'TP error fraction'
    tpMedErrorName = 'TP median per. error'
    res = pd.DataFrame(data={ m : {tpMedErrorName : np.nan, tpefName : np.nan, wmrdName: np.nan, mrdName : np.nan, corrName : np.nan, propName : np.nan} for m in (methodDict.keys() - set('Truth'))})
    ...
    for k in methodDict:
        if k.upper() != "TRUTH":
            t_name = "{}_Truth".format(measure)
            p_name = "{}_{}".format(measure, k)
            c = sp.stats.spearmanr(M["{}_Truth".format(measure)], M["{}_{}".format(measure, k)])[0]
            res[k][corrName] = c
            mrd, nonz = AnalysisUtils.relDiff("{}_Truth".format(measure), "{}_{}".format(measure, k), M)
            res[k][mrdName] = mrd["relDiff"].abs().mean()
            #weughted mard
            ind = M.index & nonz
            vals = M.loc[ind][[t_name, p_name]].max(axis=1)
            print(np.log(vals))
            res[k][wmrdName] = (mrd["relDiff"].abs() * np.log(vals)).mean()
...
def makeBenchmarkPlotSupp(timing_files, outfile):
    ...
    x_order = ['Sailfish', 'Sailfish (Quasi)', 'Salmon']
    benchmarkDF = pd.DataFrame(timingData, columns=['Organism', 'Method', 'Time (s)'])
    print( benchmarkDF )
    plt.figure(figsize=(10,6))
    sns.barplot("Method", "Time (s)", "Organism", data=benchmarkDF, x_order=x_order)
...
def qpcrplots(outpath):
    ...
    matplotlib.rc('font', **font)
    sns.set_style("white")
    tgmap = pd.read_csv('data/HumanGenomeAnnotations/tgmap.all.txt', sep='\t', names=['gene.id', 'Name', 'gene.name'])
    ...
    quantified_gene_set = set(merged.index)
    ...
    qpcr_gene_set = set(qpcrDF.index)

    # This inner join makes the comparison just on Ensembl gene names that also correspond to names in PrimePCR file
    ...
    plt.cla()
    plt.clf()
    plt.plot(merged['TPM_salmon'].rank(), merged['A'].rank(), 'o', alpha=1, ms=2)
